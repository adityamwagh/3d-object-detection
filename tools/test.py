...
def parse_args():
    ...
    parser.add_argument(
        "--eval", type=str, nargs="+", help='evaluation metrics, which depends on the dataset, e.g., "bbox",' ' "segm", "proposal" for COCO, and "mAP", "recall" for PASCAL VOC'
    )
...
def main():
    ...
    if args.gpu_ids is not None:
        ...
        warnings.warn(
            "`--gpu-ids` is deprecated, please use `--gpu-id`. " "Because we only support single GPU mode in " "non-distributed testing. Use the first GPU " "in `gpu_ids` now."
        )
    else:
    ...
    test_dataloader_default_args = dict(samples_per_gpu=1, dist=distributed, shuffle=False)
